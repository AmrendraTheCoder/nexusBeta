...
export function getContractAddress(network: string, contractName: string): string {
  const networkConfig = (DEPLOYED_CONTRACTS as any)[network];
  if (!networkConfig) {
    throw new Error(`Network ${network} not found in deployed contracts`);
  }
  
  const address = (networkConfig as any)[contractName];
  if (!address) {
...
export function getNetworkContracts(network: string) {
  const networkConfig = (DEPLOYED_CONTRACTS as any)[network];
  if (!networkConfig) {
    throw new Error(`Network ${network} not found in deployed contracts`);
  }
